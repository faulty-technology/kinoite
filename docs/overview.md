...
The client now points at radiance and caps at 4, so pointing it back at
lemonade needs the cap back at 1.
vLLM ships FP8, MTP k=4, prefix caching on, strict tool calling off.

radiance ships upstream's full-card settings, 0.98 with the measured KV pin, at
262,144 tokens, pinned to radiance-vllm-mxfp4 `dfdfa38` for its DFlash2
draft-head fix. A 0.57 / 131K footprint that freed ~13.8 GiB per card was tried
and reverted the same day: the big pool is what holds a session's conversations
...
- [ ] **The servers are tuned against one client's settings.** lemonade's
      `--parallel 4` assumes `subagent` runs one at a time, and the client
      currently runs four against radiance. Those, and advisor-M's default, live
...
- [x] **Concurrency at depth on radiance is free but not faster.** Wall clock
      ...
      buys latency shape and shallow tasks, not faster deep fan-out. The sweep
      seeded every prompt fresh, so it could not see prefix retention between
      turns, and that is what the pool turned out to be for
      [runs/2026-09-25-radiance-057-real-use-prefix-cache.md]. Mixed depths and
      prefix sharing between subagents are the other cases it could not see.
