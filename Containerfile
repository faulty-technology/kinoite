# Allow build scripts to be referenced without being copied into the final image
FROM scratch AS ctx
COPY build_files /
COPY cosign.pub /

# Base Image
FROM quay.io/fedora/fedora-kinoite:44

## Other possible base images:
# FROM quay.io/fedora/fedora-kinoite:latest  # Always latest stable Kinoite
# FROM quay.io/fedora/fedora-silverblue:44  # GNOME variant
# FROM quay.io/fedora/fedora-bootc:44       # Minimal Fedora bootc (no desktop)

### [IM]MUTABLE /opt
## Some bootable images, like Fedora, have /opt symlinked to /var/opt, in order to
## make it mutable/writable for users. However, some packages write files to this directory,
## thus its contents might be wiped out when bootc deploys an image, making it troublesome for
## some packages. Eg, google-chrome, docker-desktop.
##
## Uncomment the following line if one desires to make /opt immutable and be able to be used
## by the package manager.

RUN rm /opt && mkdir /opt

### MODIFICATIONS
## make modifications desired in your image and install packages by modifying the profile build.sh
## IMAGE_NAME is consumed by signing.sh so the image verifies against its own GHCR repo path.

ARG IMAGE_NAME=kinoite
RUN --mount=type=bind,from=ctx,source=/,target=/ctx \
    --mount=type=cache,dst=/var/cache \
    --mount=type=cache,dst=/var/log \
    --mount=type=tmpfs,dst=/tmp \
    IMAGE_NAME=${IMAGE_NAME} /ctx/profiles/base/build.sh

### LINTING
## Verify final image and contents are correct.
RUN bootc container lint
