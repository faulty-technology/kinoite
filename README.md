...
- **Streaming:** `sunshine` (via the `pvermeer/sunshine` COPR, key fingerprint-pinned — a Fedora-Atomic-targeted build with spec fixes LizardByte's own COPR lacks) enabled as a user service, plus `krfb` + `kscreen` for a KDE Wayland virtual monitor sized to the client (the Apollo-equivalent). Helper: `/usr/libexec/sunshine-virtual-display up|down`, which creates the monitor at the client's resolution *and* refresh rate and makes it primary. **This does not make the box headless** — the virtual monitor rides on top of an existing session, so a real output (dummy plug, or a connector forced on via karg) is still required; pulling the plug breaks streaming entirely. Sunshine's default `kms` capture can't see a virtual monitor and silently streams the physical panel instead, so the service seeds `capture = kwin` and `output_name = Virtual-sunshine-vm` into the per-user config on start (UI changes win), and tears the monitor down from `ExecStopPost` so a crash mid-stream can't strand it.
- **AMD tunings:** `vm.max_map_count=2147483642` sysctl (Proton games + LLM mmap), the `amdgpu.ppfeaturemask=0xfff7ffff` kernel arg baked via bootc `kargs.d` (driver default plus `PP_OVERDRIVE_MASK`, which unlocks clock/voltage offsets — narrower than the `0xffffffff` most guides repeat), and `lact` (LACT — power caps, monitoring) with its `lactd` daemon enabled. GPU fan curves don't work on the R9700 in any tool; see `notes/kinoite-north-validation.md`.
...
> - Lemonade is installed but not running: `systemctl --user start lemonade`, then `http://127.0.0.1:13305` (first start pulls a multi-GB image; the unit allows 15 minutes). GPU passthrough needs no setup — no group changes, no SELinux booleans. It stops when you log out unless you `loginctl enable-linger $USER`. If a model load dies instantly with `Memory critical error … Reason: Memory in use` and exit 134, that's SELinux denying `map` on `/dev/kfd` — check `systemctl status lemonade-selinux.service` before anything else, since nothing about the error points at permissions. Full detail in `/usr/share/kinoite/lemonade.md`.
> - Sunshine: **pairing is the only manual step.** Capture method (`kwin`), output name (`Virtual-sunshine-vm`), and the virtual-display prep commands are seeded into `~/.config/sunshine/sunshine.conf` on service start — existing settings are never overwritten, so if a stream arrives at the wrong aspect ratio, check those three under **Configuration → Advanced / General**. Making the virtual display primary only affects *new* windows — an already-running Steam stays on the desk. To pull existing windows onto it (and stream with the panels off), change the Do command to `/usr/libexec/sunshine-virtual-display up --exclusive`, globally or on a single app entry; disabling the physical outputs is what makes KWin migrate their windows. `SUNSHINE_VD_EXCLUSIVE=1` in the service environment does the same thing for every stream.
