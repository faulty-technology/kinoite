...
> - Sunshine: complete pairing, then wire the virtual display into **Configuration → General → Command Preparation** — Do: `/usr/libexec/sunshine-virtual-display up`, Undo: `/usr/libexec/sunshine-virtual-display down`. Capture method and output name are seeded already; confirm them under **Configuration → Advanced** (`kwin` / `Virtual-sunshine-vm`) if a stream comes through at the wrong aspect ratio. For headless streaming (physical panels off for the duration), add `SUNSHINE_VD_EXCLUSIVE=1` to the service environment.
