...
> - Lemonade is installed but not running: `systemctl --user start lemonade`, then `http://127.0.0.1:13305` (first start pulls a multi-GB image; the unit allows 15 minutes). Three things to know. It stops when you log out unless you `loginctl enable-linger $USER`. If it starts but sees no GPU, check `sudo ausearch -m AVC -ts recent | grep hsa_device_t` — SELinux lets containers read/write `/dev/kfd` but not `mmap` it, which ROCm needs; fix with `sudo setsebool -P container_use_devices on`. And this box exposes the iGPU to the container alongside both R9700s, which is the trigger for a known ROCm llama.cpp warmup segfault — workaround and escape hatches are in `/usr/share/kinoite/lemonade.md`.
